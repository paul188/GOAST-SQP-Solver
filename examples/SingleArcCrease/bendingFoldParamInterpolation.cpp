...
// load flat plate and prepare the arc crease
    ...
    for( int i = 0; i < plateTopol.getNumVertices(); i++ ){
        ...
        if(0 <= coords[0] && coords[0] <= 2){
            if(coords[0] <= 1.0){
                coords[0] += 0.7*coords[0]*(0.25 - std::pow(coords[1] - 0.5, 2.0));
            }
            else if(coords[0] > 1.0){
                coords[0] += 0.7*(2.0 - coords[0])*(0.25 - std::pow(coords[1] - 0.5, 2.0));
            }
        }
    ...
    OpenMesh::IO::write_mesh(plate, "bendingFoldParam_plateGeomRef.ply");
    ...
    std::vector<int> bdryMaskDirichlet2;

    // fix only x coordinates of the boundary
    std::vector<int> bdryMaskDirichlet3;
    ...
    for( int i = 0; i < plateTopol.getNumVertices(); i++ ){
        ...
        if(coords[0] >= 1.2){
            bdryMaskDirichlet2.push_back(i);
        }
        if(coords[0] == 0.0){
            bdryMaskDirichlet3.push_back(i);
        }
    ...
    OpenMesh::IO::write_mesh(plate, "deformed_first.ply");

    // CONSTRUCTION WORK STARTIN HERE CAUTION

    int numBdryNodes = bdryMaskDirichlet.size() + bdryMaskDirichlet2.size() + bdryMaskDirichlet3.size();
    std::cout<<"Individual sizes: "<<bdryMaskDirichlet.size()<<" "<<bdryMaskDirichlet2.size()<<" "<<bdryMaskDirichlet3.size()<<std::endl;
    ...
    extendBoundaryMaskPartial( plateTopol.getNumVertices(), bdryMaskDirichlet2 , active);

    std::vector<int> active2 = (std::vector<int>){1,0,0};
    extendBoundaryMaskPartial( plateTopol.getNumVertices(), bdryMaskDirichlet3 , active2);

    // append to the Dirichlet bdry mask
    bdryMaskDirichlet.insert(bdryMaskDirichlet.end(), bdryMaskDirichlet2.begin(), bdryMaskDirichlet2.end());
    bdryMaskDirichlet.insert(bdryMaskDirichlet.end(), bdryMaskDirichlet3.begin(), bdryMaskDirichlet3.end());
    ...
    for( int i = 0; i < bdryMaskDirichlet.size(); i++ )
        rhs[bdryMaskDirichlet[i]] = plateGeomDef[bdryMaskDirichlet[i]];
    ...
    // save initialization
    setGeometry( plate, plateGeomDef );
    OpenMesh::IO::write_mesh(plate, "bendingFoldParam_plateGeomDef.ply");

    //Here in the following, we use bdryMaskOpt
