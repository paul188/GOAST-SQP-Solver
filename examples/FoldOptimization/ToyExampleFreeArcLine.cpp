...
// load flat plate and prepare the arc crease
    ...
    auto foldDofsPtr = std::make_shared<FoldDofsFreeLine<DefaultConfigurator>>(plateTopol,plateGeomInitial, plateGeomInitial, bdryMaskRef, true, 0.25);

    std::vector<int> foldVertices;
    foldDofsPtr -> getFoldVertices(foldVertices);

    auto DfoldDofsPtr = std::make_shared<FoldDofsFreeLineGradient<DefaultConfigurator>>(plateTopol, bdryMaskRef, plateGeomInitial, foldVertices, true);
    ...
    ProblemDOFs<DefaultConfigurator> problemDOFs(VectorType::Ones(foldDofsPtr->getNumDofs())*t_0, plateGeomDef, foldDofsPtr, DfoldDofsPtr);
