...
// load flat plate [0,1]^2
    ...
    std::vector<int> bdryMask, bdryMaskDirichletDef_1, bdryMaskDirichletDef_x;
    for( int i = 0; i < plateTopol.getNumVertices(); i++ ){
        ...
        {
            bdryMaskDirichletDef_x.push_back( i );
        }
    }
    ...
    uniqueEntries.insert(bdryMaskDirichletDef_x.begin(), bdryMaskDirichletDef_x.end());
    ...
    setGeometry( plate, plateGeomDef );
    OpenMesh::IO::write_mesh(plate, "initPlate_0.ply");

    VectorType v_1 = plateGeomDef;

    for(int i = 0; i < plateTopol.getNumVertices(); i++){
        VecType coords;
        getXYZCoord<VectorType, VecType>( plateGeomDef, coords, i);
        coords[2] +=  3*(coords[0] - 0.5)*(coords[0] - 0.5);
        setXYZCoord<VectorType, VecType>( plateGeomDef, coords, i);
    }
