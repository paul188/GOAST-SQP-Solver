    ...
    public:
        ...
        void apply(const VectorType &vertices, RealType &Dest) const override
        {
            ...
            for(int i = 0; i < _quadTopol.getNumFaces(); i++)
            {
                ...
                Eigen::Vector3d rw_ruling_02 = 0.5*ruling_01 - 0.5*ruling_23;
...
using ADEigen3  = Eigen::Matrix<ADScalar, 3, 1>;              // 3×1 vector of AD<double>
using ADEigenVec = Eigen::Matrix<ADScalar, Eigen::Dynamic, 1>; // dynamic-length vector of AD<double>
using ADVector = CppAD::vector<ADScalar>;   // CppAD needs std::vector
...
class ConstraintSqrdReducedGradient : public BaseOp<typename ConfiguratorType::VectorType, typename ConfiguratorType::VectorType>{
    ...
    Vector computeDestAD(Vector &vertices_ad) const
    {
        ...
        // Wrap std::vector as Eigen::Matrix with Eigen::Map
        Eigen::Map<Eigen::Matrix<Real, Eigen::Dynamic, 1>> vertices_eigen(vertices_ad.data(), vertices_ad.size());

        // Same code as your function, but use AD types
        typedef Eigen::Matrix<Real, Eigen::Dynamic, 1> ADEigenVec;
        typedef Eigen::Matrix<Real, 3, 1> ADEigenVec3;
        ADEigenVec normals(3 * _num_faces);
        normals.setZero();
        ...
        for(int i = 0; i < _num_faces; i++)
        {
            ...
            Real w_0 = 0.5, w_1 = 0.5, w_2 = 0.5, w_3 = 0.5;

            ADEigenVec v_0 = vertices_eigen.segment(3*node_0,3);
            ADEigenVec v_1 = vertices_eigen.segment(3*node_1,3);
            ADEigenVec v_2 = vertices_eigen.segment(3*node_2,3);
            ADEigenVec v_3 = vertices_eigen.segment(3*node_3,3);

            ADEigenVec rw_v_0 = v_0 * w_0;
            ADEigenVec rw_v_1 = v_1 * w_1;
            ADEigenVec rw_v_2 = v_2 * w_2;
            ADEigenVec rw_v_3 = v_3 * w_3;

            ADEigenVec3 rw_e_02 = (rw_v_2 + rw_v_3) - (rw_v_0 + rw_v_1);
            ADEigenVec3 rw_e_13 = (rw_v_0 + rw_v_3) - (rw_v_1 + rw_v_2);

            ADEigenVec3 face_normal = rw_e_02.cross(rw_e_13);
            face_normal = face_normal / CppAD::sqrt(face_normal.squaredNorm()); // AD-safe norm
...
template<typename ConfiguratorType>
class ConstraintSqrdReducedHessian : public BaseOp<typename ConfiguratorType::VectorType, typename ConfiguratorType::SparseMatrixType>{
    protected:
        typedef typename ConfiguratorType::VectorType VectorType;
        typedef typename ConfiguratorType::RealType RealType;
        typedef typename ConfiguratorType::FullMatrixType FullMatrixType;
        ...
        //VectorType normals; // Store all face normals here
    ...
    public:
        ConstraintSqrdReducedHessian(const QuadMeshTopologySaver &quadTopol
                                ) : _quadTopol(quadTopol),
                                    _num_dofs(3*quadTopol.getNumVertices()),
                                    _it_face(quadTopol),
                                    _num_faces(quadTopol.getNumFaces()),
                                    _num_vertices(quadTopol.getNumVertices())
...
template<typename ConfiguratorType>
class EdgeLengthQuadEnergyHessian : public BaseOp<typename ConfiguratorType::VectorType, typename ConfiguratorType::SparseMatrixType>{
    protected:
        typedef typename ConfiguratorType::VectorType VectorType;
        typedef typename ConfiguratorType::RealType RealType;
        typedef typename ConfiguratorType::FullMatrixType FullMatrixType;
    ...
    public:
        EdgeLengthQuadEnergyHessian(const QuadMeshTopologySaver &quadTopol,
                                const VectorType &inactiveGeometry
                            ) : _quadTopol(quadTopol),
                                _inactiveGeometry(inactiveGeometry),
                                _num_dofs(3*quadTopol.getNumVertices())
        ...
        void apply(const VectorType &quad_geom, MatrixType &Dest) const override
        {
            if(Dest.rows() != _num_dofs || Dest.cols() != _num_dofs)
            {
                Dest.resize(_num_dofs, _num_dofs);
            }
            Dest.setZero();

            FullMatrixType H_dense(_num_dofs, _num_dofs);
            H_dense.setZero();

            // Compute the Hessian w.r.t. the edges
            for(int i = 0; i < _quadTopol.getNumEdges(); i++)
            {
                int node_0 = _quadTopol.getAdjacentNodeOfEdge(i, 0);
                int node_1 = _quadTopol.getAdjacentNodeOfEdge(i, 1);

                Eigen::Vector3d x_0 = quad_geom.segment(3*node_0, 3);
                Eigen::Vector3d x_1 = quad_geom.segment(3*node_1, 3);

                Eigen::Vector3d diff = x_0 - x_1;

                Eigen::Vector3d x_0_inactive = _inactiveGeometry.segment(3*node_0, 3);
                Eigen::Vector3d x_1_inactive = _inactiveGeometry.segment(3*node_1, 3);

                RealType r = (x_0 - x_1).norm();
                RealType r_sqrd = (x_0 - x_1).squaredNorm();
                RealType r_four = r_sqrd*r_sqrd;
                RealType r_inactive = (x_0_inactive - x_1_inactive).norm();

                RealType log_diff = (std::log(r) - std::log(r_inactive));

                FullMatrixType term_1 = (diff*diff.transpose())/(r_four);
                FullMatrixType term_2 = log_diff*((FullMatrixType::Identity(3,3)*(1.0/r_sqrd)) - 2.0/(r_four)*(diff*diff.transpose()));
                FullMatrixType temp = 2*(term_1 + term_2);
                H_dense.block(3*node_0, 3*node_0, 3, 3) += temp;
                H_dense.block(3*node_1, 3*node_1, 3, 3) += temp;
                H_dense.block(3*node_0, 3*node_1, 3, 3) += -temp;
                H_dense.block(3*node_1, 3*node_0, 3, 3) += -temp;
            }

            Dest = H_dense.sparseView();
        }
};

template<typename ConfiguratorType>
class EdgeLengthQuadEnergy2: public BaseOp<typename ConfiguratorType::VectorType, typename ConfiguratorType::RealType>{
    protected:
        typedef typename ConfiguratorType::VectorType VectorType;
        typedef typename ConfiguratorType::RealType RealType;
        typedef typename ConfiguratorType::FullMatrixType FullMatrixType;
    ...
    public:
        EdgeLengthQuadEnergy2(const QuadMeshTopologySaver &quadTopol,
                        const VectorType &inactiveGeometry
                    ) : _quadTopol(quadTopol),
                        _inactiveGeometry(inactiveGeometry)
        {}
        ...
        void apply(const VectorType &quad_geom, RealType &Dest) const override
        {
            ...
            for(int i = 0; i < _quadTopol.getNumEdges(); i++)
            {
                ...
                RealType term = (1 - edge_length/edge_length_inactive);
                Dest += term*term*edge_length_inactive;
            }
        }
};

template<typename ConfiguratorType>
class EdgeLengthQuadEnergyGradient2 : public BaseOp<typename ConfiguratorType::VectorType, typename ConfiguratorType::VectorType>{
    protected:
        typedef typename ConfiguratorType::VectorType VectorType;
        typedef typename ConfiguratorType::RealType RealType;
        typedef typename ConfiguratorType::FullMatrixType FullMatrixType;
    ...
    public:
        EdgeLengthQuadEnergyGradient2(const QuadMeshTopologySaver &quadTopol,
                                const VectorType &inactiveGeometry
                            ) : _quadTopol(quadTopol),
                                _inactiveGeometry(inactiveGeometry)
        {}
        ...
        void apply(const VectorType &quad_geom, VectorType &Dest) const override
        {
            ...
            for(int i = 0; i < _quadTopol.getNumEdges(); i++)
            {
                ...
                Eigen::Vector3d deriv_term = -2*(1 - edge_length/edge_length_inactive)*(1.0/edge_length)*(pos_0 - pos_1);
                Dest.segment(3*node_0, 3) += deriv_term;
                Dest.segment(3*node_1, 3) += -deriv_term;
            }
        }
