    ...
    public:
          ...
          void solve(const VectorType& plateGeomRef_basic, const std::string &filename_def_geometries, const std::string filename_ref_geometries, TriMesh& plate)
        {
            ...
            while(_pars.iter < _pars.maxIter){
                ...
                // Write out the deformed and reference geometry in this iteration
                setGeometry(plate, _problemDOFs.getDeformedGeometry());
                OpenMesh::IO::write_mesh(plate, filename_def_geometries + "plate_" + std::to_string(_pars.iter) + ".ply");
                setGeometry(plate, _problemDOFs.getReferenceGeometry());
                OpenMesh::IO::write_mesh(plate, filename_ref_geometries + "plate_" + std::to_string(_pars.iter) + ".ply");
